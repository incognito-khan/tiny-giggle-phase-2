...
export default function QuestionDetail() {
  ...
  const [ansPagination, setAnsPagination] = useState({
    page: 1,
    totalPages: 1,
  });
  ...
  const fetchAnswers = async (isLoadMore = false) => {
    try {
      ...
      const { data } = await GET(
        `/community/questions/${id}/answers?page=${nextPage}&limit=5`,
      );
      if (data.success) {
        if (isLoadMore) {
  ...
  const handlePostAnswer = async (e) => {
    ...
    try {
      setPosting(true);
      const { data } = await POST(`/community/questions/${id}/answers`, {
        content: newAnswer,
      });
      if (data.success) {
  ...
  const handleDeleteQuestion = () => {
    setDeleteModal({ open: true, type: "question", id: id });
  };

  const handleDeleteAnswer = (answerId) => {
    setDeleteModal({ open: true, type: "answer", id: answerId });
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      setIsDeleting(true);
      if (type === "question") {
        const { data } = await DELETE(`/community/questions/${targetId}`);
        if (data.success) {
          toast.success("Question deleted successfully");
          window.location.href = `/${user?.role?.toLowerCase()}-dashboard/community`;
        }
      } else {
        const { data } = await DELETE(`/community/answers/${targetId}`);
        if (data.success) {
          toast.success("Answer deleted successfully");
          setAnswers(answers.filter(a => a.id !== targetId));
          setQuestion(prev => ({
            ...prev,
            _count: {
              ...prev._count,
              answers: (prev._count.answers || 1) - 1
            }
          }));
  ...
  const handleLike = async (targetId, type = "question") => {
    ...
    try {
      const payload =
        type === "question" ? { questionId: targetId } : { answerId: targetId };
  ...
  if (!question) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white space-y-4">
        ...
        <Link href="/community">
          <Button className="rounded-xl bg-purple-600">
            Return to Community
          </Button>
        </Link>
      </div>
    ...
    <div className="min-h-screen w-full bg-[#F8FAFC] pb-20">
      ...
      <div className="bg-white/70 backdrop-blur-lg sticky top-0 z-20 border-b border-slate-100">
        <div className="max-w-5xl mx-auto px-6 h-16 flex items-center justify-between">
          <Link
            href={`/${user?.role?.toLowerCase()}-dashboard/community`}
            className="cursor-pointer"
          >
            <Button
              variant="ghost"
              className="rounded-xl gap-2 hover:bg-slate-100 font-bold transition-all"
            >
          ...
          <div className="flex gap-2">
            {question?.authorId === user?.id && (
              <Button 
                ...
                className="rounded-xl hover:bg-rose-50 text-slate-400 hover:text-rose-500"
              >
          ...
          <div className="space-y-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <Badge
                    className={`${roleInfo.bg} ${roleInfo.color} border-none font-black text-[10px] uppercase px-3`}
                  >
                    {question.authorType}
                  </Badge>
                </div>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-slate-50">
              <div className="flex items-center gap-8">
                ...
                >
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${question.isLiked ? "bg-rose-50 shadow-inner" : "bg-slate-50 group-hover:bg-rose-50"}`}
                  >
                    <Heart
                      className={`w-6 h-6 transition-colors ${question.isLiked ? "fill-rose-500 text-rose-500" : "text-slate-400 group-hover:text-rose-500"}`}
                    />
                  </div>
                  <span
                    className={`text-lg font-black transition-colors ${question.isLiked ? "text-rose-600" : "text-slate-800"}`}
                  >
      ...
      <DeleteConfirmModal
        ...
        onClose={() => !isDeleting && setDeleteModal({ open: false, type: null, id: null })}
        onConfirm={confirmDelete}
        loading={isDeleting}
        title={deleteModal.type === "question" ? "Delete Question?" : "Delete Answer?"}
        description={
          deleteModal.type === "question" 
            ? "Are you sure you want to remove this question? This will also permanently delete all associated answers."
