...
import { StatCard } from '@/components/parent-dashboard/temp/stat-card';
import { Activity, TrendingUp, Target, Syringe, Moon, Droplet, Wind, Heart, Weight, Ruler, Brain, ChevronRight, CheckCircle, Clock, AlertCircle, Utensils, Laugh, Coffee, Thermometer, Bed, Baby } from 'lucide-react';
...
export default function ParentDashboard() {
    ...
    const quickStats = [
        { icon: Bed, title: 'Last Sleep', value: formatDuration(data?.latestSleep?.duration), colorClass: 'bg-gradient-to-br from-blue-500 to-blue-600 text-white' },
        { icon: Moon, title: 'Total 7 days Sleep ', value: formatDuration(data?.totalSleepMinutesLast7Days), colorClass: 'bg-gradient-to-br from-green-500 to-green-600 text-white' },
        { icon: Baby, title: 'Current Feed', value: data?.currentFeed, subtitle: `${data?.feedPerDay} feeds per day`, colorClass: 'bg-gradient-to-br from-purple-500 to-purple-600 text-white' },
        { icon: Thermometer, title: 'Temperature', value: `${data?.latestTemperature?.temperature} °C`, subtitle: formatDate(data?.latestTemperature?.date), colorClass: 'bg-gradient-to-br from-orange-500 to-orange-600 text-white' },
    ];

    const activityFeed = [
        { icon: Utensils, title: 'Feeding', value: '6 oz formula', time: '2:30 PM', colorClass: 'bg-orange-50 border-l-4 border-orange-400' },
        { icon: Moon, title: 'Nap Time', value: '1h 15m', time: '1:45 PM', colorClass: 'bg-purple-50 border-l-4 border-purple-400' },
        { icon: Droplet, title: 'Diaper Change', value: '3 changes', time: '12:00 PM', colorClass: 'bg-green-50 border-l-4 border-green-400' },
        { icon: Wind, title: 'Play Time', value: '30 minutes', time: '11:00 AM', colorClass: 'bg-blue-50 border-l-4 border-blue-400' },
        { icon: Utensils, title: 'Feeding', value: '5 oz formula', time: '9:30 AM', colorClass: 'bg-orange-50 border-l-4 border-orange-400' },
        { icon: Laugh, title: 'Playtime & Giggles', value: 'Interactive', time: '8:15 AM', colorClass: 'bg-yellow-50 border-l-4 border-yellow-400' },
        { icon: Coffee, title: 'Wake Up', value: 'Morning', time: '7:00 AM', colorClass: 'bg-amber-50 border-l-4 border-amber-400' },
    ];

    const highlights = [
        { icon: Ruler, title: 'Height', value: data?.growthSummary?.height, subtitle: formatDate(data?.growthSummary?.date), colorClass: 'bg-orange-50 border-orange-100 text-orange-500', size: 'small' },
        { icon: Weight, title: 'Weight', value: data?.growthSummary?.weight, subtitle: formatDate(data?.growthSummary?.date), colorClass: 'bg-pink-100 border-pink-100 text-pink-500', size: 'small' },
        { icon: CheckCircle, title: 'Milestones', value: data?.milestones?.achievedMilestones, subtitle: 'Completed', colorClass: 'bg-green-50 border-green-100 text-green-500', size: 'small' },
        { icon: Syringe, title: 'Vaccines', value: `${data?.vaccinations?.achievedVaccinations || 0}/${data?.vaccinations?.totalVaccinations || 0}`, subtitle: 'On schedule', colorClass: 'bg-blue-50 border-blue-100 text-blue-500', size: 'small' },
    ];
                    ...
                    <div className='lg:col-span-2'>
                        <div>
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <h2 className="text-2xl md:text-2xl font-bold text-slate-800">Milestones Timeline</h2>
                            </div>
                            <div className="grid grid-cols-1 gap-3">
                                <Card className={`bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl border shadow-sm hover:shadow-md transition-shadow overflow-hidden`}>
                                    <div className="p-3 md:p-4 space-y-2">
                                        <div>
                                            <p className={`text-xs font-medium text-green-700`}>Recently Achieved</p>
                                            <p className="text-lg md:text-xl font-bold mt-1">{data?.milestones?.lastMilestone?.subMilestone?.title}</p>
                                            {data?.milestones?.lastMilestone?.achievedAt && <p className="text-xs mt-1 text-gray-500">{formatDate(data?.milestones?.lastMilestone?.achievedAt)}</p>}
                                        </div>
                                    </div>
                                </Card>
                                <Card className={`bg-gradient-to-br from-blue-50 to-sky-50 border-blue-200 rounded-xl border shadow-sm hover:shadow-md transition-shadow overflow-hidden`}>
                                    <div className="p-3 md:p-4 space-y-2">
                                        <div>
                                            <p className={`text-xs font-medium text-green-700`}>Upcoming</p>
                                            <p className="text-lg md:text-xl font-bold mt-1">{data?.milestones?.nextMilestone?.subMilestone?.title || <div className='text-gray-500 font-medium text-sm'>No Next Milestone</div>}</p>
                                            {data?.milestones?.nextMilestone?.achievedAt && <p className="text-xs mt-1 text-gray-500">{formatDate(data?.milestones?.nextMilestone?.achievedAt)}</p>}
                                        </div>
                                    </div>
                                </Card>
                            </div>
                        ...
                        <div className='my-6'>
                            <div className="flex items-center gap-3 mb-5">
                                ...
                                <h2 className="text-2xl md:text-2xl font-bold text-slate-800">Vaccination Timeline</h2>
                            </div>
                            <div className="grid grid-cols-1 gap-3">
                                <Card className={`bg-gradient-to-br from-green-50 to-emerald-50 border-green-200 rounded-xl border shadow-sm hover:shadow-md transition-shadow overflow-hidden`}>
                                    <div className="p-3 md:p-4 space-y-2">
                                        <div>
                                            <p className={`text-xs font-medium text-green-700`}>Last Given</p>
                                            <p className="text-lg md:text-xl font-bold mt-1">{data?.vaccinations?.lastVaccination?.vaccination?.name}</p>
                                            {data?.vaccinations?.lastVaccination?.date && <p className="text-xs mt-1 text-gray-500">Given{" "}{formatDate(data?.vaccinations?.lastVaccination?.date)}</p>}
                                        </div>
                                    </div>
                                </Card>
                                <Card className="bg-gradient-to-br from-yellow-50 to-amber-50 border-yellow-200 rounded-xl border shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                                    <div className="p-3 md:p-4 space-y-2">
                                        <p className="text-xs font-medium text-amber-700">Next Due</p>
                                        <p className="text-lg md:text-xl font-bold mt-1">
                                            {data?.vaccinations?.nextVaccination?.vaccination?.name || <div className='text-gray-500 font-medium text-sm'>No Next Vaccination</div>}
                                        </p>
                                        {data?.vaccinations?.nextVaccination?.date && (
                                            <p className="text-xs mt-1 text-gray-500">
                                                Due: {formatDate(data.vaccinations.nextVaccination.date)}
                                            </p>
                                        )}
                                    </div>
                                </Card>
                    ...
                    <div className="lg:col-span-1">
                        ...
                        <div className="space-y-4">
                            {highlights.map((highlight, idx) => {
                                ...
                                return (
                                    <div key={idx} className={`${highlight.colorClass} ${cardClasses}`}>
                                        <div className="flex items-start justify-between mb-3">
                                            <IconComponent size={highlight.size === 'large' ? 24 : 20} className="text-current opacity-90" />
                                        </div>
                                        <p className="text-sm text-slate-600 mb-2">{highlight.title}</p>
                                        <p className={`font-bold ${highlight.size === 'large' ? 'text-3xl' : 'text-2xl'} text-slate-800`}>
                                            {highlight.value}
                                        </p>
                                        <p className="text-xs text-slate-500 mt-2">{highlight.subtitle}</p>
                                    </div>
                                );
                            })}
                        </div>
