...
export async function POST(
  ...
  try {
    ...
    if (session.metadata?.type !== "BABYSITTER_ACTIVATION") {
      return Res.badRequest({ message: "Invalid payment type" });
    }

    const babysitterId = session.metadata.babysitterId;

    if (babysitterId !== user.id) {
    ...
    return Res.ok({ message: "Babysitter account activated successfully" });
  } catch (error) {
