...
export async function GET(
  ...
  try {
    ...
    const signedUrl = await signUrl(music.url);
    const signedThumbnail = await signUrl(music.thumbnail);

    return Res.success({
      message: "Music fetched successfully",
      data: { ...music, url: signedUrl, thumbnail: signedThumbnail },
    });
  } catch (error) {
  ...
  try {
    ...
    if (type === "PAID" && (!price || price <= 0)) {
      return Res.badRequest({
        message: "Please provide a valid price for PAID music",
      });
    }
    ...
    const signedUrl = await signUrl(updatedMusic.url);
    const signedThumbnail = await signUrl(updatedMusic.thumbnail);

    return Res.success({
      message: "Music updated successfully",
      data: { ...updatedMusic, url: signedUrl, thumbnail: signedThumbnail },
    });
  } catch (error) {
